...
if "arguments" in st.session_state:
    ...
    st.markdown(
        f"a. {st.session_state.shuffled_arguments[0]['text']}<br>"
        f"b. {st.session_state.shuffled_arguments[1]['text']}<br>"
        f"c. {st.session_state.shuffled_arguments[2]['text']}<br>"
        ...
        if all(st.session_state.ranking.get(k) for k in keys) and st.session_state.stance:
            ...
            if participant_id:
                ...
                collection.update_one(
                    ...
                    {"$set": {"survey_responses.1": question_response}}
                )
